    ...
    public:
        ...
        NDShape SubShape(size_t beginAxisId = 0, size_t endAxisId = SIZE_MAX) const
        {
            ...
            if ((endAxisId < beginAxisId) || (endAxisId > Rank()))
                InvalidArgument("NDShape::SubShape: The specified endAxisId (%d) must not exceed the rank (%d) of 'this' NDShape and must be >= than the specified beginAxisId (%d)", (int)endAxisId, (int)Rank(), (int)beginAxisId);
    ...
    public:
            ...
            : NDArrayView(viewShape, sourceContainer.data(), sourceContainer.size(), DeviceDescriptor::CPUDevice())
        {
            if (sourceContainer.size() != viewShape.TotalSize())
                InvalidArgument("The size (%d) of the STL container does not match the size (%d) of the specified viewShape '%S'.",
                                (int)sourceContainer.size(), (int)viewShape.TotalSize(), viewShape.AsString().c_str());
        }
...
namespace CNTK
{
    ...
    public:
        ...
        ///
        CNTK_API std::wstring AsString(bool doNotInferOutputs = false) const;

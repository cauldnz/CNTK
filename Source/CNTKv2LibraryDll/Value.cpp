...
namespace CNTK
{
    ...
    static NDMaskPtr CreateMask(const std::vector<size_t>& sequenceLengths, const std::vector<bool>& sequenceStartFlags, const DeviceDescriptor& device)
    {
        ...
        if (!sequenceStartFlags.empty() && (sequenceStartFlags.size() != numSequences))
            InvalidArgument("Value::Create:: The number (%d) of sequence start flags does not match the number (%d) of sequences,",
                            (int)sequenceStartFlags.size(), (int)numSequences);
        ...
        for (size_t i = 0; i < numSequences; ++i)
        {
            ...
            for (; j < currentSequenceLength; ++j)
            {
                ...
                if (oneHotSequences[i][j] >= dimension)
                    InvalidArgument("Value::Create: one-hot index value (%d) exceeds vocabulary size (%d).", (int)oneHotSequences[i][j], (int)dimension);
    ...
    /*static*/ ValuePtr Value::Create(const NDShape& sampleShape, const std::vector<NDArrayViewPtr>& sequences, const std::vector<bool>& sequenceStartFlags, const DeviceDescriptor& device, bool readOnly, bool createNewCopy)
    {
        ...
        for (size_t i = 0; i < numSequences; ++i)
        {
            ...
            if ((currentSequenceDataShape.Rank() < sampleShape.Rank()) || (currentSequenceDataShape.Rank() > (sampleShape.Rank() + 1)) || (currentSequenceDataShape.SubShape(0, sampleShape.Rank()) != sampleShape))
                InvalidArgument("Value::Create: The shape '%S' of the sequence %lu is not compatible with the sample shape '%S'", currentSequenceData->Shape().AsString().c_str(), (unsigned long)i, sampleShape.AsString().c_str());
        ...
        if (isDataSparse && (sampleShape[0] != sampleShape.TotalSize()))
            InvalidArgument("Value::Create: For sparse data, the sample shape '%S' %s axis dimensionality must equal the total size (%d) of the sample.",
                            sampleShape.AsString().c_str(), Internal::IsReversingTensorShapesInErrorMessagesEnabled() ? "trailing" : "leading", (int)sampleShape.TotalSize());
    ...
    /*static*/ ValuePtr Value::Create(const NDShape& sampleShape, const std::vector<std::vector<ElementType>>& sequences, const std::vector<bool>& sequenceStartFlags, const DeviceDescriptor& device, bool readOnly)
    {
        ...
        for (size_t i = 0; i < numSequences; ++i)
        {
            ...
            if ((currentSequence.size() % numElementsPerSample) != 0)
                InvalidArgument("Value::Create: The number of elements (%d) in the vector containing sequence data must be a multiple of the size (%d) of specified sample shape '%S'",
                                (int)currentSequence.size(), (int)numElementsPerSample, sampleShape.AsString().c_str());
    ...
    /*static*/ ValuePtr Value::CreateBatch(const NDShape& sampleShape, const std::vector<ElementType>& batchData, const DeviceDescriptor& device, bool readOnly /*= false */)
    {
        ...
        if (batchData.size() % shapeSize != 0)
            InvalidArgument("The number of elements (%d) in the vector containing batch data must be a multiple of the size (%d) of the sample shape '%S'.",
                            (int)batchData.size(), (int)shapeSize, sampleShape.AsString().c_str());
    ...
    /*static*/ ValuePtr Value::CreateSequence(const NDShape& sampleShape, const std::vector<ElementType>& sequenceData, bool sequenceStartFlag, const DeviceDescriptor& device, bool readOnly /*= false */)
    {
        ...
        if (sequenceData.size() % shapeSize != 0)
            InvalidArgument("The number of elements (%d) in the sequence data must be a multiple of the size (%d) of the sample shape '%S'", 
                            (int)sequenceData.size(), (int)shapeSize, sampleShape.AsString().c_str());
    ...
    void Value::CopyVariableValueToVector(const Variable& outputVariable, std::vector<std::vector<size_t>>& sequences)
    {
        if (outputVariable.Shape()[0] != outputVariable.Shape().TotalSize())
            InvalidArgument("For sparse data, the outputVariable's leading axis dimensionality (%d) must equal the total size (%d) of the Variable '%S'.",
                            (int)outputVariable.Shape()[0], (int)outputVariable.Shape().TotalSize(), outputVariable.AsString().c_str());
    ...
    void Value::CopyVariableValueToImpl(const Variable& outputVariable, std::vector<std::vector<DestType>>& sequences)
    {
        ...
        if (sequences.size() < numOfSequences)
            RuntimeError("The size of output buffer (%d) is smaller than the number (%d) of sequences.", (int)sequences.size(), (int)numOfSequences);
    ...
    void DirectCopy(const ElementType *source, const size_t elementCount, std::vector<DestType>& dest)
    {
        ...
        if (elementCount > dest.size())
            RuntimeError("Copy: The output buffer size (%d) is smaller than the number (%d) of source elements to copy.", (int)dest.size(), (int)elementCount);

                                              ...
                                              const DeviceDescriptor& device /*= DeviceDescriptor::CPUDevice()*/)
    {
        ...
        for (auto& currentStreamKV : computedMeanAndInvStdDevs)
        {
            ...
            if (minibatchSourceStreams.find(currentStreamInfo) == minibatchSourceStreams.end())
                InvalidArgument("Stream '%S' for which mean and variance is to be computed is not supported by the specified minibatchSource.", currentStreamKV.first.AsString().c_str());

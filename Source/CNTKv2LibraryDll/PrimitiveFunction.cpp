                ...
                case PrimitiveOpType::FutureValue:
                {
                    ...
                    if ((m_op == PrimitiveOpType::PastValue) || (m_op == PrimitiveOpType::FutureValue))
                    {
                        ...
                        if ((inputOperandVar.DynamicAxes() != Axis::UnknownDynamicAxes()) && (inputOperandVar.DynamicAxes().size() != 2))
                            LogicError("PastValue/FutureValue Function '%S': Input operand '%S' with #dynamic axes != 2 (1 sequence axis and 1 batch axis) is not supported.", AsString(/*doNotInferOutputs =*/ true).c_str(), inputOperandVar.AsString().c_str());
                    }
                    ...
                    if (!anyInputShapesUnknown)
                    {
                        ...
                        case PrimitiveOpType::TransposeAxes:
                        {
                            ...
                            if (!axis1.IsStaticAxis() || !axis2.IsStaticAxis())
                                LogicError("Function '%S': TransposeAxes operation currently does not support transposing dynamic axes.", AsString(/*doNotInferOutputs =*/ true).c_str());
                        ...
                        case PrimitiveOpType::Slice:
                        {
                            ...
                            if (!axis.IsStaticAxis())
                                LogicError("Function '%S': Built-in Slice operation currently does not support slicing along dynamic axis.", AsString(/*doNotInferOutputs =*/ true).c_str());
                        ...
                        case PrimitiveOpType::Unpooling:
                        {
                            ...
                            if (unpoolingType != PoolingType::Max)
                                LogicError("Function '%S': Currently only max unpooling is supported.", AsString(/*doNotInferOutputs =*/ true).c_str());
                            ...
                            if (inferredInputShape != inputShape)
                                RuntimeError("Unpooling: The shape '%S' of the unpooling operand '%S' is different than the resultng shape '%S from pooling the pooling input argument '%S' using the provided options.", 
                        ...
                        case PrimitiveOpType::TransposeTimes:
                        {
                            ...
                            if (m_inputs[0].Shape().Rank() > 2)
                                LogicError("Function '%S': TransposeTimes operation currently requires the %s operand '%S' to be of rank 1 or 2", AsString(/*doNotInferOutputs =*/ true).c_str(), Internal::IsReversingTensorShapesInErrorMessagesEnabled() ? "right" : "left", m_inputs[0].AsString().c_str());
                        ...
                        case PrimitiveOpType::Splice:
                        {
                            ...
                            if (!spliceAxis.IsStaticAxis())
                                LogicError("Function '%S': Splice operation currently does not support splicing along dynamic axis", AsString(/*doNotInferOutputs =*/ true).c_str());

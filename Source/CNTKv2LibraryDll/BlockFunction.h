...
namespace CNTK
{
    ...
    private:
        /*static*/ std::vector<Variable> DetermineInputs(const FunctionPtr& composite, const std::vector<std::pair<Variable, Variable>>& argumentsMap, const std::wstring& blockName) const
        {
            ...
            {
                InvalidArgument("%d of the arguments '%S' of the underlying composite Function of Block '%S' have not been provided mappings for when encapsulating the composite as a Block.",
                                (int)unmappedArguments.size(), NamedListString(unmappedArguments).c_str(), blockName.c_str());
            }

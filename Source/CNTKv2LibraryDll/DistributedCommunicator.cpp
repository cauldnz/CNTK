        ...
        for (auto i = 0; i < numValues; ++i)
        {
            ...
            if (m_nccl->IsSupported())
            {
                if (dataType == DataType::Float)
                    m_nccl->AllReduceImpl(inputData, outputData, numElements, Microsoft::MSR::CNTK::NcclComm::DataType::FLOAT);
                else if (dataType == DataType::Double)
                    m_nccl->AllReduceImpl(inputData, outputData, numElements, Microsoft::MSR::CNTK::NcclComm::DataType::DOUBLE);
                else
